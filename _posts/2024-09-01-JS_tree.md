...
Now if we want to find the values of $p$ for which there is a non-negative value for $Q_A$, the probability of Aaron winning the game from the first move, then we need to find the smallest value of $p$ for which $$p^6 Q_A^3 - 2p^3Q_A + 1 = 0$$ has a positive solution when we solve for $Q_A$.
...
$\hat{p} = \frac{3}{2^{5/3}}$, we see $Q_A$ has near repeated roots of around $\approx 0.88$, validating our solution. Interestingly the probability of winning is zero until $p > \hat{p}$ at which it jumps up to around $0.88$, as seen in the plot above! 
