...
roughly like $$ \sim \frac{n}{3}$$. In fact, we can actually note that $$\frac{n}{3}$$ is a lower bound on the optimal expected score and furthermore it can be *obtained with certainty* by choosing one color of rabbit and guessing that color each turn until the game ends. 
